...
def parse_args(argv: List[str]) -> argparse.Namespace:
    ...
    color_follow = sub.add_parser("color-follow", help="Start the color-based object tracking custom feature.")
    color_follow.add_argument(
        "--target-color",
        default="red",
        choices=["red", "blue", "green", "yellow", "orange"],
        help="Target color to track.",
    )
...
def main(argv: List[str] | None = None) -> None:
    ...
    try:
        ...
        if args.command == "color-follow":
            extra = [f"target_color:={args.target_color}"]
            _ros2_launch("turtlebot3_automation", "color_follow.launch.py", extra)
            return
